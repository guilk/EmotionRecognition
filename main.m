...
if prepare_mode == 1
    ...
    [ck_features, ck_labels] = prepare_CK_data(ck_root, ck_samples, ck_inds);
    bu3dfe_inds = 1:length(bu3dfe_samples);
    [bu3dfe_features, bu3dfe_labels] = prepare_BU3DFE_data(bu3dfe_root, bu3dfe_samples, bu3dfe_inds);
    bosphorus_inds = 1:length(bosphorus_samples);
    [bosphorus_features, bosphorus_labels] = prepare_Bosphorus_data(bosphorus_root, bosphorus_samples, bosphorus_inds);
    
    tr_features = [ck_features; bu3dfe_features; bosphorus_features];
    tr_labels = [ck_labels; bu3dfe_labels; bosphorus_labels];
    ...
    save('./models/all_model.mat','model');
    ...
    tr_labels = double(tr_labels);
    model = svmtrain(tr_labels, tr_features, opt);
    save('./models/pca_all_model.mat','model');
